...
输入：
- 可调度事件：/home/deep/TimeSeries/Agent_V2/experiments/Robustness/01PowerMeasurementNoise/output/04_tou_optimization_filter/UK/{Economy_7|Economy_10}/{house_id}/

输出：
- 调度结果：/home/deep/TimeSeries/Agent_V2/experiments/Robustness/01PowerMeasurementNoise/output/05_Initial_scheduling_optimization/{tariff_name}/{house_id}/
"""
...
# 路径配置
NOISE_TOU_FILTER_DIR = os.path.join(EXPERIMENT_DIR, "output/04_tou_optimization_filter")  # 噪声TOU过滤结果
NOISE_OUTPUT_DIR = os.path.join(EXPERIMENT_DIR, "output")  # 噪声实验输出目录
ORIGINAL_CONSTRAINTS_DIR = os.path.join(BASE_DIR, "output/04_user_constraints")  # 原始用户约束
...
class EventScheduler:
    ...
    def get_appliance_spaces_path(self, house_id: str, season: str = None) -> str:
        """获取电器工作空间文件路径 - 使用错误约束文件"""
        # 🎯 修改为使用错误约束文件路径
        base_path = "/home/deep/TimeSeries/Agent_V2/experiments/Robustness/03Constraint_Parsing_Errors/Error_data/UK"

        # 解析tariff_name
        if "/" in self.tariff_name:
            region, tariff = self.tariff_name.split("/", 1)
            if tariff == "TOU_D" and season:
                return f"{base_path}/TOU_D/{season}/{house_id}/appliance_reschedulable_spaces.json"
            elif tariff in ["Economy_7", "Economy_10"]:
                return f"{base_path}/{tariff}/{house_id}/appliance_reschedulable_spaces.json"
            else:
                return f"{base_path}/{tariff}/{house_id}/appliance_reschedulable_spaces.json"
        else:
            if self.tariff_name == "TOU_D" and season:
                ...
                return f"{base_path}/{self.tariff_name}/{house_id}/appliance_reschedulable_spaces.json"
            else:
                return f"{base_path}/{self.tariff_name}/{house_id}/appliance_reschedulable_spaces.json"

    def get_events_path(self, house_id: str) -> str:
        """🎯 功率测量噪声实验 - 获取可调度事件文件路径"""
        # 🎯 使用功率测量噪声实验的TOU过滤结果
        ...
        # 🎯 构建噪声实验的路径模式
        patterns = [
            f"{NOISE_TOU_FILTER_DIR}/{region}/{tariff}/{house_id}/*.csv",
            f"{NOISE_TOU_FILTER_DIR}/{region}/{tariff}/{house_id}/tou_filtered_{house_id}_{tariff}.csv"
        ]

        for pattern in patterns:
            files = glob.glob(pattern)
            if files:
                print(f"📁 找到事件文件: {files[0]}")
                return files[0]

        print(f"❌ 未找到事件文件，搜索路径: {patterns}")
        return None

    def get_output_path(self, house_id: str, season: str = None) -> str:
        """🎯 功率测量噪声实验 - 获取输出文件路径"""
        # 🎯 输出到功率测量噪声实验目录
        base_path = NOISE_OUTPUT_DIR

        # 解析tariff_name
        if "/" in self.tariff_name:
            region, tariff = self.tariff_name.split("/", 1)
            if tariff == "TOU_D" and season:
                return f"{base_path}/05_Initial_scheduling_optimization/TOU_D/{season}/{house_id}/scheduled_events.csv"
            elif tariff in ["Economy_7", "Economy_10"]:
                return f"{base_path}/05_Initial_scheduling_optimization/{tariff}/{house_id}/scheduled_events.csv"
            else:
                return f"{base_path}/05_Initial_scheduling_optimization/{tariff}/{house_id}/scheduled_events.csv"
        else:
            if self.tariff_name == "TOU_D" and season:
                ...
                return f"{base_path}/05_Initial_scheduling_optimization/{self.tariff_name}/{house_id}/scheduled_events.csv"
            else:
                return f"{base_path}/05_Initial_scheduling_optimization/{self.tariff_name}/{house_id}/scheduled_events.csv"
...
def run_robustness_experiment():
    """运行约束解析错误鲁棒性实验 - 无交互模式"""
    print("🚀 约束解析错误鲁棒性实验 - Event Scheduler")
    print("=" * 60)
    ...
    for tariff in tariff_list:
        ...
        for house_id in target_houses:
            ...
            result = process_single_house(tariff, house_id)
            tariff_results[house_id] = result

            if result["status"] == "success":
                successful_houses += 1
                print(f"   ✅ 成功: {result['successful_events']}/{result['total_events']} 事件调度成功")
            else:
        ...
        all_results[tariff] = {
            "status": "success",
            "tariff_name": tariff,
            "total_houses": len(target_houses),
            "successful_houses": successful_houses,
            "results": tariff_results
        }

        print(f"\n📊 {tariff} 汇总: {successful_houses}/{len(target_houses)} 家庭处理成功")
    ...
    for tariff, tariff_data in all_results.items():
        successful = tariff_data["successful_houses"]
        total = tariff_data["total_houses"]
        total_successful += successful
        total_houses += total

        print(f"✅ {tariff}: {successful}/{total} 家庭成功")

    overall_success_rate = total_successful / total_houses * 100 if total_houses > 0 else 0
    print(f"\n🎯 总体成功率: {total_successful}/{total_houses} ({overall_success_rate:.1f}%)")
