...
# 路径配置
NOISE_EVENT_SEGMENTS_DIR = os.path.join(EXPERIMENT_DIR, "output/02_event_segments")  # 噪声事件分割结果
...
def get_appliance_list_from_csv(
    ...
    house_id: str = "",
    tariff_type: str = "UK",
    output_dir: str = None  # 将使用噪声实验输出目录
) -> Optional[Dict]:
    """
    🎯 功率测量噪声实验 - 从事件分割CSV提取电器列表
    ...
    Args:
        ...
        output_dir: Output directory (will use noise experiment output if None)
    ...
    if output_dir is None:
        output_dir = os.path.join(NOISE_OUTPUT_DIR, "04_appliance_summary")
...
def batch_get_appliance_lists(
    ...
    tariff_type: str = "UK",
    input_dir: str = None,  # 将使用噪声事件分割目录
    output_dir: str = None  # 将使用噪声实验输出目录
) -> Dict[str, Dict]:
    """
    🎯 功率测量噪声实验 - 批量处理电器列表提取
    ...
    Args:
        ...
        tariff_type: Type of tariff ("UK", "Germany", "California")
        input_dir: Directory containing event segments (will use noise experiment if None)
        output_dir: Output directory (will use noise experiment if None)
    ...
    if input_dir is None:
        input_dir = NOISE_EVENT_SEGMENTS_DIR
    if output_dir is None:
        output_dir = os.path.join(NOISE_OUTPUT_DIR, "04_appliance_summary")
    ...
    print(f"🚀 功率测量噪声实验 - 批量电器列表提取...")
    print(f"📊 电价类型: {tariff_type}")
    print(f"🏠 目标房屋数: {len(house_data_dict)}")
...
def single_house_appliance_analysis(
    ...
    print(f"🏠 功率测量噪声实验 - {house_id.upper()} 电器分析...")
    print(f"📊 电价类型: {tariff_type}")
    print(f"📁 输入目录: {input_dir}")
    print(f"📁 输出目录: {output_dir}")
    print("=" * 60)
