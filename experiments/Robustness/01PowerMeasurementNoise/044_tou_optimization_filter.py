...
ORIGINAL_TARIFF_DIR = os.path.join(BASE_DIR, "config/tariff")  # 原始电价配置
ORIGINAL_APPLIANCE_SUMMARY_DIR = os.path.join(BASE_DIR, "output/04_appliance_summary")  # 原始电器摘要
...
def process_and_mask_events(
    ...
    if tariff_config_path is None:
        ...
        if tariff_name == "TOU_D":
            # 🎯 For California TOU_D, use absolute path to config file
            possible_paths = [
                os.path.join(BASE_DIR, "config/TOU_D.json"),
                "./config/TOU_D.json",
                "../config/TOU_D.json",
            ...
            # 🎯 For Germany tariffs, use absolute path to config file
            possible_paths = [
                os.path.join(BASE_DIR, "config/Germany_Variable.json"),
            ...
            # 🎯 For UK tariffs, use absolute path to config file
            possible_paths = [
                os.path.join(BASE_DIR, "config/tariff_config.json"),
        ...
        if tariff_config_path is None:
            ...
            if tariff_name == "TOU_D":
                tariff_config_path = os.path.join(BASE_DIR, "config/TOU_D.json")
            elif tariff_name in ["Germany_Variable", "Germany_Variable_Base"]:
                tariff_config_path = os.path.join(BASE_DIR, "config/Germany_Variable.json")
            else:
                tariff_config_path = os.path.join(BASE_DIR, "config/tariff_config.json")
...
def process_single_household_complete_pipeline(
    ...
    """🎯 功率测量噪声实验 - 单个房屋TOU过滤完整流程"""

    # 🎯 使用功率测量噪声实验的路径配置
    if min_duration_input_dir is None:
        min_duration_input_dir = os.path.join(NOISE_OUTPUT_DIR, "04_min_duration_filter")
    if constraints_dir is None:
        constraints_dir = os.path.join(BASE_DIR, "output/04_user_constraints")
    if output_dir is None:
        output_dir = os.path.join(NOISE_OUTPUT_DIR, "04_tou_optimization_filter")
        for path in possible_paths:
            if os.path.exists(path):
