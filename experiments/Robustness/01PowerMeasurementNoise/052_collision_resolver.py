#!/usr/bin/env python3
"""
🎯 功率测量噪声鲁棒性实验 - Collision Resolver
...
📁 输入路径: /home/deep/TimeSeries/Agent_V2/experiments/Robustness/01PowerMeasurementNoise/output/05_Initial_scheduling_optimization/{tariff_name}/house*/scheduled_events.csv
📁 输出路径: /home/deep/TimeSeries/Agent_V2/experiments/Robustness/01PowerMeasurementNoise/output/05_Collision_Resolved_Scheduling/{tariff_name}/house*/scheduled_events.csv

作者: Agent V2
日期: 2025-08-07
版本: 2.0 - 支持批处理 - 功率测量噪声实验版本
"""
...
# 路径配置
NOISE_OUTPUT_DIR = os.path.join(EXPERIMENT_DIR, "output")  # 噪声实验输出目录
...
class P052CollisionResolver:
    """P052调度结果冲突解决器 - 批量处理多个房屋"""

    def __init__(self, input_dir: str = None, output_dir: str = None):
        # 🎯 使用功率测量噪声实验的路径配置
        if input_dir is None:
            input_dir = os.path.join(NOISE_OUTPUT_DIR, "05_Initial_scheduling_optimization")
        if output_dir is None:
            output_dir = os.path.join(NOISE_OUTPUT_DIR, "05_Collision_Resolved_Scheduling")
    ...
    def detect_collisions_in_group(self, group_df: pd.DataFrame) -> List[Tuple[int, int]]:
        ...
        # 按调度后的开始时间排序 - 使用正确的列名
        if 'scheduled_start_time' not in group_df.columns:
            print("⚠️ 警告: 缺少 scheduled_start_time 列，无法检测冲突")
            return collisions

        sorted_events = group_df.sort_values('scheduled_start_time').reset_index()
        ...
        for i in range(len(sorted_events) - 1):
            ...
            current_end = pd.to_datetime(current_event['scheduled_end_time'])
            next_start = pd.to_datetime(next_event['scheduled_start_time'])
    ...
    def resolve_collisions_for_house(
        ...
        # 按电器和日期分组处理SUCCESS事件
        # 添加日期列用于分组
        df_success['event_date'] = pd.to_datetime(df_success['scheduled_start_time']).dt.date
        groups = df_success.groupby(['appliance_name', 'event_date'])

        processed_groups = 0
        for (appliance_name, date), group in groups:
            if len(group) <= 1:
                continue  # 单个事件无冲突

            # 检测真正的时间重叠冲突
            collisions = self.detect_collisions_in_group(group)
            ...
            # 统计真正的冲突数量
            stats['conflicts_detected'] += len(collisions)

            # 收集所有已调度的时间段
            occupied_slots = []
            for idx in group.index:
                start_time = pd.to_datetime(df_success.loc[idx, 'scheduled_start_time'])
                end_time = pd.to_datetime(df_success.loc[idx, 'scheduled_end_time'])
                occupied_slots.append((start_time, end_time))

            # 处理每个冲突对
            for collision_idx1, collision_idx2 in collisions:
                # 选择保留优先级较高的事件（通常是较早的事件）
                event1_id = df_success.loc[collision_idx1, 'event_id']
                event2_id = df_success.loc[collision_idx2, 'event_id']

                # 简单策略：保留第一个事件，重新调度第二个事件
                idx_to_reschedule = collision_idx2

                # � 简化输出：不显示每个事件的重新调度详情

                original_start = pd.to_datetime(df_success.loc[idx_to_reschedule, 'original_start_time'])
                original_end = pd.to_datetime(df_success.loc[idx_to_reschedule, 'original_end_time'])
                event_duration = int((original_end - original_start).total_seconds() / 60)

                # 使用约束空间寻找可用时间段
                original_price_level = int(df_success.loc[idx_to_reschedule, 'original_price_level'])
                ...
                if new_slot:
                    ...
                    df_success.loc[idx_to_reschedule, 'scheduled_start_time'] = new_start
                    df_success.loc[idx_to_reschedule, 'scheduled_end_time'] = new_end

                    # 更新占用时间段列表
                    old_start = pd.to_datetime(df_success.loc[idx_to_reschedule, 'scheduled_start_time'])
                    old_end = pd.to_datetime(df_success.loc[idx_to_reschedule, 'scheduled_end_time'])
                    occupied_slots = [(start, end) for start, end in occupied_slots
                                    if not (start == old_start and end == old_end)]
                    ...
                    # 无法找到合适时间段，标记为失败
                    df_success.loc[idx_to_reschedule, 'schedule_status'] = 'FAILED'
                    df_success.loc[idx_to_reschedule, 'failure_reason'] = 'No available time slot after collision resolution'
