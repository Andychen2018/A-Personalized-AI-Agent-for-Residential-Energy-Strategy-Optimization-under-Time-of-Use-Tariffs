#!/usr/bin/env python3
"""
时间不确定性鲁棒性实验 - Cost Calculator

在不修改其它代码的前提下：
- 读取 event_splitter 产出的 migrated/non_migrated 事件文件
- 结合 01_preprocessed 的每分钟瞬时功率，按所选电价方案的区间将能量/费用分摊
- 未迁移事件计算一次（原始时间窗）
- 成功迁移事件计算两次（迁移前：original，迁移后：scheduled）
- 支持 Economy_7/Economy_10 电价方案
- 缺失分钟功率按0处理

输出：experiments/Robustness/02Timing_Uncertainties/output/06_cost_cal/
  Economy_7/houseX/{migrated_costs.csv, non_migrated_costs.csv}
  Economy_10/houseX/{migrated_costs.csv, non_migrated_costs.csv}
...
# 输入路径：使用功率测量噪声实验的事件分割结果
SPLIT_BASE = os.path.join(OUTPUT_BASE, '05_event_split')

# 输出路径：功率测量噪声实验的费用计算结果
COST_BASE = os.path.join(OUTPUT_BASE, '06_cost_cal')

# 🎯 噪声数据路径（用于获取添加扰动后的功率数据）
NOISE_DATA_BASE = os.path.join(BASE_DIR, 'Noise_data')

# 原始数据路径（备用）
ORIGINAL_BASE_DIR = "/home/deep/TimeSeries/Agent_V2"

# 配置文件路径
TARIFF_CFG = os.path.join(ORIGINAL_BASE_DIR, 'config', 'tariff_config.json')
TOU_D_CFG = os.path.join(ORIGINAL_BASE_DIR, 'config', 'TOU_D.json')
GER_VAR_CFG = os.path.join(ORIGINAL_BASE_DIR, 'config', 'Germany_Variable.json')
...
def list_houses() -> List[str]:
    houses = []
    if os.path.exists(NOISE_DATA_BASE):
        for d in os.listdir(NOISE_DATA_BASE):
            if d.startswith('house') and os.path.isdir(os.path.join(NOISE_DATA_BASE, d)):
                houses.append(d)
    def hnum(h):
...
def load_power_timeseries(house_id: str) -> pd.DataFrame:
    ...
    # 🎯 使用噪声数据路径
    path = os.path.join(NOISE_DATA_BASE, house_id, f"01_perception_alignment_result_{house_id}_noisy.csv")
...
def process_house_tariff(house_id: str, tariff: str, scope: str):
    ...
    # 简要统计
    non_migrated_total_cost = float(df_non_out['total_cost'].sum() if 'total_cost' in df_non_out else 0.0)
    migrated_total_cost = float(df_mig_out.filter(like='sched_total_cost').sum().sum())

    # 🎯 计算整个房屋的总费用（基于实际功率数据）
    house_total_cost = calc_house_total_cost(house_id, tariff)
    ...
    stats = {
        ...
        'non_cost': non_migrated_total_cost,
        ...
        'mig_sched_cost': migrated_total_cost,
        # 🎯 使用整个房屋的总费用
        'non_migrated_total_cost': non_migrated_total_cost,
        'migrated_total_cost': migrated_total_cost,
        'events_total_cost': non_migrated_total_cost + migrated_total_cost,  # 仅事件费用
        'house_total_cost': house_total_cost,  # 整个房屋费用
        'total_cost': house_total_cost  # 使用房屋总费用作为主要费用
    }

    return stats


def calc_house_total_cost(house_id: str, tariff_type: str) -> float:
    """计算整个房屋的总电费（基于实际功率数据）"""
    # 🎯 加载噪声功率数据
    power_file = os.path.join(NOISE_DATA_BASE, house_id, f'01_perception_alignment_result_{house_id}_noisy.csv')

    if not os.path.exists(power_file):
        print(f"⚠️ 功率文件不存在: {power_file}")
        return 0.0
    ...
    try:
        ...
        if tariff_type == 'Economy_7':
            # Economy_7: 夜间(23:00-07:00) 0.15p/kWh, 其他时间 0.30p/kWh
            night_mask = (df_power['hour'] >= 23) | (df_power['hour'] < 7)
            night_energy_wh = df_power[night_mask]['Aggregate'].sum() / 60  # W -> Wh
            day_energy_wh = df_power[~night_mask]['Aggregate'].sum() / 60

            night_energy_kwh = night_energy_wh / 1000  # Wh -> kWh
            day_energy_kwh = day_energy_wh / 1000

            night_cost = night_energy_kwh * 0.15  # £
            day_cost = day_energy_kwh * 0.30
            total_cost = night_cost + day_cost

        elif tariff_type == 'Economy_10':
            # Economy_10: 夜间(00:00-05:00, 13:00-16:00, 20:00-22:00) 0.15p/kWh, 其他时间 0.30p/kWh
            cheap_mask = ((df_power['hour'] >= 0) & (df_power['hour'] < 5)) | \
                        ((df_power['hour'] >= 13) & (df_power['hour'] < 16)) | \
                        ((df_power['hour'] >= 20) & (df_power['hour'] < 22))

            cheap_energy_wh = df_power[cheap_mask]['Aggregate'].sum() / 60
            expensive_energy_wh = df_power[~cheap_mask]['Aggregate'].sum() / 60

            cheap_energy_kwh = cheap_energy_wh / 1000
            expensive_energy_kwh = expensive_energy_wh / 1000

            cheap_cost = cheap_energy_kwh * 0.15
            expensive_cost = expensive_energy_kwh * 0.30
            total_cost = cheap_cost + expensive_cost

        else:
            # 标准费率 0.30p/kWh
            total_energy_wh = df_power['Aggregate'].sum() / 60
            total_energy_kwh = total_energy_wh / 1000
            total_cost = total_energy_kwh * 0.30

        return total_cost

    except Exception as e:
        print(f"⚠️ 计算房屋总费用失败 {house_id} {tariff_type}: {e}")
        return 0.0
    ...
    for hid in target_houses:
        ...
        for tariff, scope in tasks:
            ...
            try:
                ...
                events_total_cost = st.get('events_total_cost', 0)
                house_total_cost = st.get('house_total_cost', 0)

                print(f"      ✅ 完成 {tariff}")
                print(f"         迁移事件费用: £{migrated_cost:.2f}")
                print(f"         未迁移事件费用: £{non_migrated_cost:.2f}")
                print(f"         事件总费用: £{events_total_cost:.2f}")
                print(f"         整个房屋总费用: £{house_total_cost:.2f}")
