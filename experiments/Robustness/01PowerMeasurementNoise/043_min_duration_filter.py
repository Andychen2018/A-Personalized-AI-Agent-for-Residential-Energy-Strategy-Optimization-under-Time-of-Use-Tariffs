...
class MinDurationEventFilter:
    ...
    def load_appliance_constraints(self, house_id: str, constraints_dir: str = None) -> Optional[Dict]:
        ...
        if constraints_dir is None:
            constraints_dir = ORIGINAL_CONSTRAINTS_DIR
    ...
    def process_single_household(self, house_id: str,
        ...
        """🎯 功率测量噪声实验 - 单个房屋最小持续时间过滤"""
        
        # 🎯 使用功率测量噪声实验的路径配置
        if events_dir is None:
            events_dir = NOISE_EVENT_SEGMENTS_DIR
        if output_dir is None:
            output_dir = os.path.join(NOISE_OUTPUT_DIR, "04_min_duration_filter")

        print(f"🏠 功率测量噪声实验 - {house_id.upper()} 最小持续时间过滤...")
    ...
    def process_batch_households(self, house_list: List[str],
        ...
        print(f"🚀 功率测量噪声实验 - 批量最小持续时间过滤...")
        print(f"🏠 目标房屋数: {len(house_list)}")
        print(f"📁 约束目录: {constraints_dir}")
        print(f"📁 事件目录: {events_dir}")
        print(f"📁 输出目录: {output_dir}")
        print("=" * 80)
