#!/usr/bin/env python3
"""
时间不确定性鲁棒性实验 - Event Scheduler
基于电器工作空间的事件调度优化器（适配时间扰动数据）

功能：
1. 读取正确的电器工作空间约束文件
2. 使用时间扰动后的事件数据进行调度
3. 基于价格等级和约束进行事件调度优化
4. 支持Economy_7和Economy_10电价方案
5. 输出到时间不确定性实验专用目录

输入：
- 电器工作空间约束：experiments/Robustness/03Constraint_Parsing_Errors/Original_data/UK/{tariff_name}/{house_id}/
- 扰动后事件数据：experiments/Robustness/02Timing_Uncertainties/Error_data/UK/{tariff_name}/{house_id}/

输出：
- 调度结果：experiments/Robustness/02Timing_Uncertainties/output/05_Initial_scheduling_optimization/{tariff_name}/{house_id}/
"""
...
class EventScheduler:
    ...
    def __init__(self, tariff_name: str, house_id: str = None):
        ...
        Args:
            tariff_name: 电价方案名称 (Economy_7 或 Economy_10)
        ...
        self.perturbed_data_dir = os.path.join(self.base_dir, "Error_data/UK")
    ...
    def get_appliance_spaces_path(self, house_id: str, season: str = None) -> str:
        """获取电器工作空间文件路径 - 使用正确的约束文件"""
        # 🎯 时间不确定性实验：使用正确的约束文件（不是错误约束）
        return f"{self.constraint_dir}/{self.tariff_name}/{house_id}/appliance_reschedulable_spaces.json"

    def get_events_path(self, house_id: str) -> str:
        """获取可调度事件文件路径 - 使用时间扰动后的数据"""
        # 🎯 时间不确定性实验：使用扰动后的事件数据
        return f"{self.perturbed_data_dir}/{self.tariff_name}/{house_id}/tou_filtered_{house_id}_{self.tariff_name}.csv"

        # 构建路径模式
        patterns = [
            f"{base_original_path}/04_TOU_filter/{region}/{tariff}/{house_id}/*.csv",
            f"{base_original_path}/04_user_constraints/{house_id}/shiftable_event_masked_{tariff}.csv"
        ]
    ...
    def get_output_path(self, house_id: str, season: str = None) -> str:
        """获取输出文件路径 - 输出到时间不确定性实验目录"""
        # 🎯 时间不确定性实验：输出到专用目录
        return f"{self.output_dir}/05_Initial_scheduling_optimization/{self.tariff_name}/{house_id}/scheduled_events.csv"
...
def run_robustness_experiment():
    """运行时间不确定性鲁棒性实验 - 无交互模式"""
    print("🚀 时间不确定性鲁棒性实验 - Event Scheduler")
    print("=" * 60)
