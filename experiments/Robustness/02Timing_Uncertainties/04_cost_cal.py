...
def load_power_timeseries(house_id: str) -> pd.DataFrame:
    ...
    path = os.path.join(PREPROC_BASE, house_id, f"01_perception_alignment_result_{house_id}.csv")
...
def calc_house_total_cost(house_id: str, tariff_type: str) -> float:
    """计算整个房屋的总电费（基于实际功率数据）"""
    # 加载功率数据
    power_file = os.path.join(PREPROC_BASE, house_id, f'01_perception_alignment_result_{house_id}.csv')
