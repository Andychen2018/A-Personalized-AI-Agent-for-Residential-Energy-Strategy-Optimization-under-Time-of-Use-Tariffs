...
PREPROC_BASE = os.path.join(OUTPUT_BASE, '01_preprocessed')
...
def process_house_tariff(house_id: str, tariff: str, scope: str):
    ...
    if tariff in ['Economy_7','Economy_10']:
        in_dir = os.path.join(SPLIT_BASE, 'UK', scope, house_id)
        out_dir = os.path.join(COST_BASE, 'UK', scope, house_id)
...
def calc_standard_cost(house_id: str, tariff_group: str) -> Dict:
    ...
    if tariff_group == 'UK':
        ...
        for sub_tariff in ['Economy_7', 'Economy_10']:
            non_path = os.path.join(SPLIT_BASE, 'UK', sub_tariff, house_id, 'non_migrated_events.csv')
            mig_path = os.path.join(SPLIT_BASE, 'UK', sub_tariff, house_id, 'migrated_events.csv')
